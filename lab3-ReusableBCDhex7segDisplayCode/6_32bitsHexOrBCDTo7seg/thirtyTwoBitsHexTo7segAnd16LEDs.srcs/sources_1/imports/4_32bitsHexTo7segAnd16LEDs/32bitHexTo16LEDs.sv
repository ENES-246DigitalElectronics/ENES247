module thirtyTwobitHexTo16LEDs(
    ...
    wire [31:0] BCD ; //this is creats the BCD version of C_input
    reg start = 0; // start Hex to BCD conversion .. whch could take 40 clock ticks or more
    wire done; // driven by Binary_to_BCD is 0 when working 1 when done
    
    Binary_to_BCD i1(clk,c_input[27:0],start,BCD,done);
    assign o_done = done;
    ...
    begin
        if (reset == 1) begin
            ...
            if (count_clk == divider_counter-1) begin
                count_clk <= 0;
                c_input <= c_input + 1;
                if (HEX_BCD && done) start<=1; else start<=0; //this starts the hex to bcd conversion
                o_start=start;
                end         
    ...
    always_comb if (HEX_BCD == 1) hex_to_display = BCD[4*segment +: 4];     
    else hex_to_display = c_input[4*segment +: 4];  // 3 bits of anode_select, grab 4 bits of c_input and put them in hex_to_display
